 ...
 * Cho phép Admin điều chỉnh các mức điểm thưởng trong hệ thống
 */
const PointConfiguration = () => {
  // CÁC TRẠNG THÁI (STATE)
  const [configs, setConfigs] = useState([]); // Danh sách các cấu hình từ server
  const [loading, setLoading] = useState(true); // Trạng thái đang tải dữ liệu
  const [saving, setSaving] = useState(false); // Trạng thái đang lưu dữ liệu lên server

  // Hàm lấy toàn bộ cấu hình từ API
  ...
  // Gọi fetchConfigs ngay khi component được gắn (mount)
  useEffect(() => {
    fetchConfigs();
  }, []);

  // Hàm xử lý khi người dùng thay đổi giá trị điểm trên ô input (thay đổi tạm thời ở local)
  const handleValueChange = (key, newValue) => {
    setConfigs(configs.map(c => c.key === key ? { ...c, value: newValue } : c));
  };

  // Hàm xử lý khi người dùng nhấn nút "Lưu" để cập nhật lên database thông qua API
  const handleSave = async (key, value) => {
    try {
      setSaving(true);
      await updateConfig(key, value);
      toast.success('Lưu cấu hình thành công!');
      fetchConfigs(); // Tải lại dữ liệu mới nhất sau khi lưu
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 lg:p-8 transition-all duration-300">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* BANNER THÔNG TIN GIỚI THIỆU (INFO BANNER) */}
        ...
        {/* THẺ CHỨA DANH SÁCH CẤU HÌNH (CONFIGURATION CARD) */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
          <div className="p-6 md:p-8">
            ...
            {configs.length === 0 ? (
              ...
              <div className="space-y-4">
                {/* Lọc và hiển thị cấu hình cụ thể cho việc hoàn thành báo cáo */}
                {configs
                  ...
                  <div key={config.key} className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 p-5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                    ...
                    {/* KHU VỰC NHẬP DỮ LIỆU ĐIỂM (INPUT GROUP) */}
          ...
          {/* PHẦN CHÂN TRANG (FOOTER) CỦA CARD */}
